module faucet::f {
    ...
    public entry fun initialize(account: &signer, initial_balance: u64) {
        ...
        assert!(account_addr == @faucet, E_NOT_OWNER);
    ...
    public entry fun claim(account: &signer, amount: u64) acquires FaucetData {
        let account_addr = signer::address_of(account);
        assert!(exists<FaucetData>(@faucet), E_NOT_INITIALIZED);
        assert!(amount > 0, E_INVALID_AMOUNT);
        
        let faucet_data = borrow_global_mut<FaucetData>(@faucet);
    ...
    public fun get_balance(): u64 acquires FaucetData {
        assert!(exists<FaucetData>(@faucet), E_NOT_INITIALIZED);
        let faucet_data = borrow_global<FaucetData>(@faucet);
